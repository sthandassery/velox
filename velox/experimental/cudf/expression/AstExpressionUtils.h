...
struct AstContext {
  ...
  /// Determine which side (0=left, 1=right) an expression references by
...
cudf::ast::expression const& AstContext::pushExprToTree(
  ...
  auto usePreResolvedSubExpr = [&]() -> cudf::ast::expression const& {
    auto it = resolvedSubExprs.find(expr.get());
    VELOX_CHECK(
        it != resolvedSubExprs.end(),
        "Sub-expression not resolved by compiler: {}",
        expr->toString());
    ...
    auto node = it->second;
    // For two-table reference (hash join), the pre-resolved expression was
    // compiled against the concatenated schema.  Re-compile against the
    // side-specific schema so column indices match correctly.
    if (inputRowSchema.size() > 1) {
      node = ExpressionTreeCompiler::compile(expr, inputRowSchema[sideIdx]);
    }
    return addPrecomputeInstructionOnSide(
  ...
  switch (expr->kind()) {
    ...
    // Fallback: delegate to pre-resolved sub-expression from the compiler.
    default: {
      return usePreResolvedSubExpr();
    }
  }
